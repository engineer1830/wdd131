...
async function fetchFundData(ticker) {
    ...
    else if (t.includes("BND") || t.includes("AGG") || t.includes("BOND")) {
        avgAnnual = 0.04;
    }
    else {
        avgAnnual = 0.08;
...
async function financialPerformance(tickers) {
    ...
    const avgConservative =
        conservative.length > 0
            ? conservative.reduce((total, [, returns]) => total + returns, 0) / conservative.length
            : 0.0;

    const avgModerateAggressive =
        moderateAggressive.length > 0
            ? moderateAggressive.reduce((total, [, returns]) => total + returns, 0) /
